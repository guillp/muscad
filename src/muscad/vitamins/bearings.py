...
class LinearBearing(Part):
    ...
    def add_throats(self, diameter: float, width: float, offset: float) -> LinearBearing:
        throat = Cylinder(d=self._outer_diam + 1, h=width) - Cylinder(d=diameter, h=width + 1)
    ...
    def add_rod_clearance(self, length: float = 20, T: float = 1) -> LinearBearing:
        self.rod_clearance = Cylinder(d=self._inner_diam + 2 * T, h=self._length + 2 * length).misc()
        return self
...
class BushingLinearBearing(Part):
    ...
    def add_rod_clearance(
        ...
        slide = slide or {}
        self.rod_clearance = (
            Cylinder(d=self.rod_diameter + 2 * T, h=self.width + 2 * length).slide(**slide).x_rotate(90).misc()
        )
        return self
...
class RotationBearing(Part):
    ...
    def add_clearance(self, size: float) -> RotationBearing:
        self.clearance = ~(Cylinder(d=self._outer_diameter + size, h=self._height) - self.inner)
        return self
