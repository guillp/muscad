...
def test_minkowski() -> None:
    m = Minkowski(
        Cube(2, 5, 7).align(left=2, back=3, bottom=4),
        Cylinder(h=7, d=2).align(left=3, back=4),
    )
    assert (
        m.render()
        == """\
minkowski()
{
  translate(v=[3.0, 5.5, 7.5])
  cube(size=[2, 5, 7], center=true);
  translate(v=[4.0, 5.0, 0])
  cylinder(h=7, d=2, $fn=15, center=true);
    ...
    assert m.back == 7
    assert m.front == 14
    assert m.center_y == 10.5
    assert m.bottom == 0.5
    assert m.top == 14.5
    assert m.center_z == 7.5
...
def test_linear_extrusion() -> None:
